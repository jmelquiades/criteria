...
class Report_Libro_Mayor(models.AbstractModel):
    ...
    def _get_report_name(self):
        return ("Reporte de libro Mayor")
    
    @api.model
    def _get_templates(self):
        templates = super(Report_Libro_Mayor, self)._get_templates()
        templates['line_template'] = 'addcri_account_report_libro_mayor.line_template'
        templates['main_template'] = 'addcri_account_report_libro_mayor.main_template'
        return templates

    @api.model
    def _get_columns_name(self, options):
        columns_names = [
            {'name': _('LIBRO'),'style':'text-align: center;border: 1px solid #e6e6e6'},
            {'name': _('TD')},
            {'name': _('NUMERO')},
            {'name': _('NRO CORRELATIVO')},
            {'name': _('FECHA'), 'class': 'date'},
            {'name': _('DESCRIPCION O GLOSA')},
    ...
    def _get_general_ledger_lines(self, options, line_id=None):
        ...
        for account, periods_results in accounts_results:
            ...
            if has_lines and (unfold_all or is_unfolded):
                ...
                # Totales de Cuenta
                lines.append(self._get_total_Cuenta(
                    options,account,
                    'Total Cuenta:',
                    debit,
                    credit,
    ...
    def _get_options_domain(self, options):
        # OVERRIDE
        domain = super(Report_Libro_Mayor, self)._get_options_domain(options)
    ...
    def _get_query_amls_select_clause(self):
        return '''
            ...
            latam_document_type.code				AS code_type_document,
            account_move_line__move_id.voucher_number AS voucher_number,
            account_move_line__move_id.glosa		AS glosa
        '''
    ...
    def _get_query_amls_from_clause(self):
        return '''
            ...
            LEFT JOIN l10n_latam_document_type latam_document_type ON latam_document_type.id = account_move_line__move_id.l10n_latam_document_type_id 
        '''
    ...
    def _get_account_title_line(self, options, account, amount_currency, debit, credit, balance, has_lines):
        ...
        name = 'Cod. Cuenta: %s %s' % (account.code, account.name)
        columns = [
            {'name': ''},
            {'name': ''},
    ...
    @api.model
    def _get_total_Cuenta(self, options,account,name,  debit, credit):
        columns = [
    ...
    def _get_aml_line(self, options, account, aml, cumulated_balance):
        ...
        columns = [
            {'name': aml['code_type_document'], 'class': 'o_account_report_line_ellipsis'},
            {'name': aml['voucher_number'], 'class': 'o_account_report_line_ellipsis'},
            {'name': aml['move_name'], 'class': 'o_account_report_line_ellipsis'},
            {'name': format_date(self.env, aml['date']), 'class': 'date'},
            {'name': aml['glosa'], 'class': 'o_account_report_line_ellipsis'},
    ...
    def _get_tax_declaration_lines(self, options, journal_type, taxes_results):
        lines = [{
            ...
            'columns': [{'name': ''}],
            # 'colspan': self.user_has_groups('base.group_multi_currency') and 4 or 5,
            'colspan':  5,
            'level': 1,
            'unfoldable': False,
            'unfolded': False,
        ...
        for tax_line in tax_report_lines:
            if tax_line['id'] not in ('sale', 'purchase'): # We want to exclude title lines here
                ...
                # tax_line['colspan'] = self.user_has_groups('base.group_multi_currency') and 4 or 5
                tax_line['colspan'] = 5
