    ...
    def _compute_name(self):
        ...
        for batch in final_batches:
            for move in batch['records']:
                ...
                # Give the name based on previously computed values for journal with sequence
                else:
                    move.name = self.env['ir.sequence'].next_by_code(move.journal_id.sequence_id.code)
            batch['records'].filtered(lambda m: not m.journal_id.sequence_id)._compute_split_sequence()
