...
class ResCurrencyRate(models.Model):
    ...
    def _execute_compute_exchange_rate_moves(self, vals):
        ...
        company = vals.get('company_id', self.company_id.id)
        currency = vals.get('currency_id', self.currency_id.id)
        ...
        domain = [('date', '>=', date_str), ('company_id', '=', company), ('currency_id', '=', currency)]

        next_dates = self.env['res.currency'].browse(currency).rate_ids.filtered(lambda cr: cr.name > date).sorted(lambda d: d.name)
