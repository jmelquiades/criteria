...
class ResCompany(models.Model):
    ...
    currency_provider = fields.Selection(
        selection_add=[('sunat', "SUNAT")], default='sunat')
    ...
    def _parse_sunat_data(self, available_currencies):
        """SUNAT Peru (SUNAT)
        API Doc: https://api.apis.net.pe/v1/tipo-cambio-sunat?fecha=2021-06-23
        Source: https://apis.net.pe/api-tipo-cambio.html

            Solo dólares

        """

        sunat_date_format_url = '%Y-%m-%d'
        sunat_date_format_res = '%Y-%m-%d'
        result = {}
        available_currency_names = available_currencies.mapped('name')
        if 'PEN' not in available_currency_names:
            return result
        ...
        url_format = "https://api.apis.net.pe/v1/tipo-cambio-sunat?fecha=%(date)s"
        ...
        first_pe_str = (date_pe - datetime.timedelta(days=7))
        second_pe_str = date_pe

        dates = [first_pe_str + datetime.timedelta(days=d) for d in range((second_pe_str - first_pe_str).days + 1)]
        
        for date_pe_str in dates:
            if 'USD' not in available_currency_names:
                continue
            data = {
            'date': date_pe_str.strftime(sunat_date_format_url),
            }
            url = url_format % data
            try:
                res = requests.get(url, timeout=10)
                ...
                _logger.error(e)
                continue
            date_rate_str = series['fecha']
            fetched_rate = [float(series['venta']), float(series['compra'])]
            rate = [1.0 / rate if rate else 0 for rate in fetched_rate]
            if not rate:
                continue

            date_rate = datetime.datetime.strptime(date_rate_str, sunat_date_format_res).strftime(DEFAULT_SERVER_DATE_FORMAT)
    ...
    def _generate_currency_rates(self, parsed_data):
        ...
        for company in self:
            ...
            for currency, (rate, date_rate) in parsed_data.items():
                ...
                elif isinstance(rate, (list)) and len(rate) == 2:

                    currency_object = Currency.search([('name', '=', currency)])
                    if currency_object:  # if rate provider base currency is not active, it will be present in parsed_data
                        already_existing_rate = CurrencyRate.search([('currency_id', '=', currency_object.id), ('name', '=', date_rate), ('company_id', '=', company.id)])
                        if already_existing_rate:
                            already_existing_rate.rate = rate[0] / base_currency_rate
                            already_existing_rate.purchase_rate = rate[1] / base_currency_rate
                        else:
                            rate_value = rate[0] / base_currency_rate
                            purchase_rate_value = rate[1] / base_currency_rate
                            CurrencyRate.create({'currency_id': currency_object.id, 'rate': rate_value, 'purchase_rate': purchase_rate_value, 'name': date_rate, 'company_id': company.id})
